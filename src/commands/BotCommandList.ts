...
export class BotCommandList {
	...
	public get() {
		let commandGroups = groupByKey(this.commands, "lang");
		return commandGroups.map(cmdGroup => ({
			language: cmdGroup.key,
			commands: groupByKey(cmdGroup.values, "scope").map(
				cmd => ({
					scope: cmd.key,
	...
	public implementDecorators(...decoratedHandlers: Record<string, any>[]) {
		for (let handler of decoratedHandlers) {
			let metadata: CommandInfoDecoratorMetadata =
				handler?.constructor?.prototype?.__tgCommandInfo;
	...
	public setScope(
		type: IBotCommandScope["type"],
		params?: Omit<IBotCommandScope, "type">
	) {
